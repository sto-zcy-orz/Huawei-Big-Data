...
num = 0
train = pd.DataFrame(columns=['id', 'loadingOrder', 'first_longitude', 'first_latitude'])
print(len(file_list))
for file in file_list:
    ...
    train_o = pd.DataFrame({
        ...
        'first_longitude': [round(train_data_o['longitude'][0])],
        'first_latitude': [round(train_data_o['latitude'][0])]
    })
    train = train.append(train_o, ignore_index=True)

#train.to_csv('last_zuobiao.csv', index=True)
train.to_csv('first_zuobiao.csv', index=True)
print(train)
