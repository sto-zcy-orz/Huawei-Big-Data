...
num = 0
train = pd.DataFrame(columns = ['id', 'loadingOrder', 'carrierName', 'timestamp', 'longitude',
...
for file in file_list:
    ...
    if debug:
        train_data_o = pd.read_csv(file_path, nrows=NDATA, header=None)
    else:
        train_data_o = pd.read_csv(file_path, header=None)
    train_data_o.columns = ['id', 'loadingOrder', 'carrierName', 'timestamp', 'longitude',
                          'latitude', 'vesselMMSI', 'speed', 'direction', 'vesselNextport',
                          'vesselNextportETA', 'vesselStatus', 'vesselDatasource', 'TRANSPORT_TRACE']
    train_o = get_feature(get_data(train_data_o, mode='train'), mode='train')
    if (num == 1):
        train = train_o
    else:
